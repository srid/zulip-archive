...
data Page
  ...
  | Page_StreamTopic (Target Stream) (Target Topic)
  ...
  streamsT <- forM streams $ \stream -> do
    ...
    case _streamTopics stream of
      ...
      Just topics -> forM_ topics $ \topic -> do
        ...
        Rib.writeHtml g $ renderPage $ Page_StreamTopic streamT topicT
    pure streamT
...
pageName = \case
  ...
  Page_StreamTopic _s (Rib.targetVal -> t) -> _topicName t

pageUrl :: Page -> Text
pageUrl = \case
  Page_Index _ -> "/"
  Page_Stream s -> Rib.targetUrl s
  Page_StreamTopic _s t -> Rib.targetUrl t
...
renderPage page = with html_ [lang_ "en"] $ do
  head_ $ do
    ...
    title_ $ case page of
      ...
      Page_StreamTopic (Rib.targetVal -> s) (Rib.targetVal -> t) -> do
        toHtml $ _topicName t
        " - "
        toHtml $ _streamName s
              ...
              $ \topic -> with div_ [class_ "item"] $ do
                ...
                with div_ [class_ "content"] $ do
                  ...
                  with div_ [class_ "description"] $ do
                    ...
                    " messages."
          Page_StreamTopic _stream (Rib.targetVal -> topic) -> do
  ...
  where
    ...
    pageTitle = toHtml . \case
      ...
      Page_StreamTopic (Rib.targetVal -> s) (Rib.targetVal -> t) -> _topicName t <> " - " <> _streamName s
